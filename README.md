...
Under the hood: zero external requests on Levels 1/3, a rule-based retrieval engine that refuses instead of hallucinating (committed regression suites — `test/retrieval.test.mjs` 53, `test/region.test.mjs` 9 regions across 19 timezones, and three relay suites via `cd relay && npm test`: 201 + 13 + 8), and a codename email relay on **Render** with crisis interception, school-address-only delivery, and spoof-resistant rate limiting.
